...
public class DataServiceIMP implements DataService{
        ...
        for(int i=0;i<= dtoItems.size()-1;i++){
            ...
            if(AFN == (byte)0X0C){                             //一类数据
                ...
                if(commandItemCode.equals("100C0073")){//模拟量实时数据（温度）
                    insertData_HUMITURE(dto.getLogicAddress(),dtoItem.gp,"0",dataItemMap.get("100C007301"));
                }
            }
            ...
            if(AFN == (byte)0X10){                         //透明转发数据
                if(commandItemCode.equals("8000B66F")){
                    String phase = dataItemMap.get("8000B66F03");
    ...
    {
        try {
            this.getHumitureStoredProcedure().execute(logicalAddress, gpSn, humiture, temperature);
        } catch (Exception e) {
            log.error("错误信息：", e.fillInStackTrace());
        }
    }
