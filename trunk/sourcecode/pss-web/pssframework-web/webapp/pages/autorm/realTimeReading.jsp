...
function read(commanditems) {
    ...
    $.ajax({
        ...
        success: function(data) {
            fetchReturnResult(data.collectId, 10, commanditems);
        },
        error: function() {
        }
    });
...
<div class="electric_lcon" id="electric_Con">
  <ul class=default id=electric_Con_1>
    ...
    <div id="tbl-container" style="height: expression(((document.documentElement.clientHeight ||document.body.clientHeight)-133));">
      <table width="100%" border="0" cellspacing="0" class="gridBody" id="object_table">
        <thead class="tableHeader">
          <tr>
            ...
            <th sortColumn="ctTimes" >CT变比</th>
            <th sortColumn="ptTimes" >PT变比</th>
          </tr>
        </thead>
        <tbody class="tableBody" id="dataBody">
          ...
          <tr class="${status.count % 2 == 0 ? 'odd' : 'even'}">
            ...
            <td id="<c:out value="${item.logicalAddr}"/>_<c:out value="${item.gpSn}"/>_ctTimes"><c:out value='${item.ctTimes}' />&nbsp;</td>
            <td id="<c:out value="${item.logicalAddr}"/>_<c:out value="${item.gpSn}"/>_ptTimes"><c:out value='${item.ptTimes}' />&nbsp;</td>
          </tr>
          </c:forEach>
        </tbody>
      </table>
...
function viewTds(data, commanditems, fetchCount) {
    if(data != null) {
        ...
        for(var i = 0; i < jsonObj.data.length; i++) {
            ...
            try {
            	ida = keyArray[0] + "_" + keyArray[1];
            	code = keyArray[2];
            	ctTimes = parseFloat($("#" + ida + "_ctTimes").html());
            	ptTimes = parseFloat($("#" + ida + "_ptTimes").html());
        	}
        	catch(e) {
        		ctTimes = 1;
        		ptTimes = 1;
                ...
                if($("#" + key).html() != null && $("#" + key).html() == "正在发送...") {
                    if(otherDataFlag == "1") { //如果是其他数据项，展示曲线数据
                       insertCurData(key,value);
                    	...
                    	if("2101" == code || "2102" == code || "2103" == code) {
                    		value = parseFloat(value) * ptTimes;
                    		value = value.toFixed(2);
                    	}
                    	if("2201" == code || "2202" == code || "2203" == code) {
                    		value = parseFloat(value) * ctTimes;
                    		value = value.toFixed(2);
                    	}
                    	if("0100" == code || "0101" == code || "0102" == code || "0103" == code || "0104" == code || "A000" == code || "0200" == code || "A100" == code) {
                    		value = parseFloat(value) * 1;
                            value = value.toFixed(2);
                    	}
                    	if("2300" == code || "2400" == code) {
                    		value = parseFloat(value) * ctTimes * ptTimes;
                    		value = value.toFixed(2);
                    	}
                        $("#" + key).html(value);
                    }
