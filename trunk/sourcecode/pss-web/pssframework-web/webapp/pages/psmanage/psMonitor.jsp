...
function showResultReadB66F(resultMap) {
    ...
    if(typeof result != "undefined") {
        ...
        for(var i = 0; i < $("input[ci='8000B66F']").length; i++) {
            //alert($($("input[ci='8000B66F']")[i]).attr("di"));
            key = $($("input[ci='8000B66F']")[i]).attr("di");
            //alert(result[$($("input[ci='8000B66F']")[i]).attr("di")]);
            val = result[$($("input[ci='8000B66F']")[i]).attr("di")];
            $($("input[ci='8000B66F']")[i]).val(result[$($("input[ci='8000B66F']")[i]).attr("di")]);
            if('B611' == key || 'B612' == key || 'B613' == key) {       // 分相电压
                if(typeof val != 'undefined' && val != null && val != 'null' && val != '' && val != ' ') {
                    if(parseFloat(val) == 0.0) {
                        $($("input[ci='8000B66F']")[i]).val('/');
                    }
                }
            }
            else if('B621' == key || 'B622' == key || 'B623' == key) {  // 分相电流
                if(typeof val != 'undefined' && val != null && val != 'null' && val != '' && val != ' ') {
                    if(parseFloat(val) == 0.0) {
                        $($("input[ci='8000B66F']")[i]).val('<10');
                    }
                }
            }
            else if('B660' == key) {                                    // 剩余电流
                if(typeof val != 'undefined' && val != null && val != 'null' && val != '' && val != ' ') {
                    if(parseFloat(val) == 0.0) {
                        $($("input[ci='8000B66F']")[i]).val('<10');
                    }
