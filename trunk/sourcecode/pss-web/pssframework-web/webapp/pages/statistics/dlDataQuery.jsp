  ...
  <table width="100%" border="0" cellspacing="0" class="gridBody" id="object_table">
    <thead class="tableHeader">
      <tr>
        ...
        <th sortColumn="pactTotalLast">电量起码(kWh)</th>
        <th sortColumn="pactTotal">电量止码(kWh)</th>
        <th sortColumn="pactDl">用电量(kWh)</th>
      </tr>
    </thead>
    ...
    <tbody class="tableBody" id="dataBody" >
      <c:forEach items="${page.result}" var="item" varStatus="status">
        <tr class="${status.count % 2 == 0 ? 'odd' : 'even'}">
          ...
          <td><c:out value='${item.pactTotalLast}' />&nbsp;</td>
          <td><c:out value='${item.pactTotal}' />&nbsp;</td>
          <td><c:out value='${item.pactDl}' />&nbsp;</td>
        </tr>
      </c:forEach>
    </tbody>
  </table>
