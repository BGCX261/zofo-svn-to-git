 ...
 */
var SimpleTable = function(formId, pageNumber, pageSize, sortColumns, exportReportUrl, pageNumberKey, pageSizeKey, sortColumnsKey) {
	this.ctx = '';
	this.form = formId;
	...
	// 1-需要导出 ，其他为不需要
	this.exportReportUrl = exportReportUrl || '/exportReport';
...
SimpleTable.prototype = {
	...
	exportReport : function() {
		...
		$("#loading").show();
		if(!formAction) {
			formAction = $('#' + this.form).attr("action");
		}
		if(!excelAction) {
			excelAction = this.ctx + this.exportReportUrl;
		}
		//$('#' + this.form).attr("action", excelAction);
		//location.href = url;
		SimpleTableUtils.fireExportSubmit(this.form);
...
var SimpleTableUtils = {
	...
	fireExportSubmit : function(_form) {
		if(excelAction) {
			$('#' + _form).attr("action", excelAction);
		}
		var form = document.getElementById(_form);
		if (form.fireEvent) { // for ie
			if (form.fireEvent('onsubmit')) {
